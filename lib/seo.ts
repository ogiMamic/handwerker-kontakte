...
// Durchschnittliche Stundensätze pro Gewerk (Fallback wenn keine DB-Daten)
...
// Bayern Städte haben tendenziell höhere Preise
...
export function generateSEOContent(
  ...
  if (gewerk) {
    return generateGewerkPage(stadtName, bundesland, gewerkLabel, gewerk, preisMin, preisMax, stats);
  }
  return generateStadtPage(stadtName, bundesland, stadtSlug, stats);
}

function generateStadtPage(
  stadtName: string,
  bundesland: string,
  stadtSlug: string,
  stats?: { anzahl: number; avgBewertung: number }
): SEOContent {
  ...
  return {
    ...
    faq: [
      {
        frage: `Was kostet ein Handwerker in ${stadtName}?`,
        antwort: `Die Kosten hängen vom Gewerk und Aufwand ab. Stundensätze in ${stadtName} liegen typischerweise zwischen 35 € und 95 €. Für ein genaues Angebot empfehlen wir, direkt Kontakt aufzunehmen.`,
      },
      {
        frage: `Wie finde ich einen guten Handwerker in ${stadtName}?`,
        antwort: `Auf handwerker-kontakte.de können Sie Handwerker nach Bewertung, Gewerk und Preis filtern. Achten Sie auf verifizierte Betriebe und lesen Sie die Erfahrungsberichte anderer Kunden.`,
      },
      {
        frage: `Wie schnell bekomme ich einen Termin in ${stadtName}?`,
        antwort: `Die Wartezeit variiert je nach Gewerk und Saison. In der Regel können Sie innerhalb von 1–2 Wochen einen Termin erhalten. Bei Notfällen bieten einige Handwerker auch Sofortservice an.`,
      },
    ],
  };
}

function generateGewerkPage(
  stadtName: string,
  bundesland: string,
  gewerkLabel: string,
  gewerk: GewerkType,
  ...
  return {
    ...
    faq: [
      {
        frage: `Was kostet ein ${gewerkLabel} in ${stadtName} pro Stunde?`,
        antwort: `${gewerkLabel} in ${stadtName} berechnen typischerweise zwischen ${preisMin} € und ${preisMax} € pro Stunde. Hinzu kommen ggf. Materialkosten und Anfahrt.`,
      },
      {
        frage: `Woran erkenne ich einen guten ${gewerkLabel}?`,
        antwort: `Achten Sie auf: Eintrag in der Handwerksrolle, Meisterbrief, positive Bewertungen, transparente Preisgestaltung und eine ordentliche Gewerbeanmeldung.`,
      },
      {
        frage: `Brauche ich für ${gewerkLabel}-Arbeiten eine Genehmigung?`,
        antwort: `Das hängt von der Art der Arbeit ab. Größere Umbauarbeiten erfordern oft eine Baugenehmigung. Ihr ${gewerkLabel} kann Sie hierzu beraten.`,
      },
    ],
  };
}
...
export function generateJsonLd(
  stadtName: string,
  gewerk?: string,
  handwerkerList?: { firma: string; bewertung_avg: number; bewertung_count: number }[]
) {
  const baseUrl = 'https://handwerker-kontakte.de';
  ...
  const jsonLd: any = {
    ...
    '@type': 'WebPage',
    name: gewerk
      ? `${gewerk} in ${stadtName} finden`
      : `Handwerker in ${stadtName} finden`,
    url: `${baseUrl}/handwerker/${stadtName.toLowerCase()}${gewerk ? `/${gewerk}` : ''}`,
    description: `Finden Sie geprüfte Handwerker in ${stadtName}. Bewertungen, Preise, direkter Kontakt.`,
    mainEntity: {
      '@type': 'ItemList',
      itemListElement:
        handwerkerList?.map((h, i) => ({
          '@type': 'ListItem',
          position: i + 1,
          item: {
            '@type': 'LocalBusiness',
            name: h.firma,
            aggregateRating: h.bewertung_count > 0
              ? {
                  '@type': 'AggregateRating',
                  ratingValue: h.bewertung_avg,
                  reviewCount: h.bewertung_count,
                  bestRating: 5,
                }
              : undefined,
            areaServed: {
